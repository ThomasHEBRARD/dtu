...
def OPT(cri):
    if cri == 0:
        ...
        for OPT_DATA in OPT(cri - 1):
            ...
            c_2_v = [
                D[cri][pi] if pi in arr_excl([optc[0], optc[1]]) else None for pi in R
            ]
            case_2_max = max([a for a in c_2_v if a is not None])
            case_2_index = c_2_v.index(case_2_max)

            c_3_v = [
                D[cri][pi] if pi in arr_excl([optc[0], optc[2]]) else None for pi in R
            ]
            case_3_max = max([a for a in c_3_v if a is not None])
            case_3_index = c_3_v.index(case_3_max)

            c_4_v = [
                D[cri][pi] if pi in arr_excl([optc[1], optc[2]]) else None for pi in R
            ]
            ...
            so = sorted(list_duplicates(D[cri]))
            for s in so:
                ind = list(s)[1]
                for i in ind:
            ...
            return [[current_total, cas] for cas in counter_case_C]
