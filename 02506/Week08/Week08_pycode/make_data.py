...
def make_data(example_nr, n=200, noise=1):
    ...
    if example_nr == 1:
        X = np.r_[
            noise * 10 * np.random.randn(K[0], 2) + np.array([30, 30]),
        ...
        X = (
            np.r_[
                noise * 5 * np.random.randn(K[0], 2)
                + 30 * np.array([np.cos(rand_ang), np.sin(rand_ang)]).T,
                noise * 5 * np.random.randn(K[1], 2),
            ]
            + dim / 2
        )
    elif example_nr == 3:
        X = np.r_[
            noise * 10 * np.random.randn(int(K[0] / 2), 2) + np.array([30, 30]),
