...
img3 = cv2.drawMatchesKnn(
    im1, kp1, im2, kp2, good, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS
)
...
ang = 67
sc = 0.6
imr = scipy.ndimage.rotate(scipy.ndimage.zoom(im1, sc), ang, reshape=False)[
    50:-50, 50:-50
]
plt.imshow(imr)
...
def plot_matching_keypoints(im1, im2, pts_im1, pts_im2):
    ...
    ax.plot(
        np.c_[pts_im1[0], pts_im2[0] + c1].T,
        np.c_[pts_im1[1], pts_im2[1]].T,
        "w",
        linewidth=0.5,
    )
...
# Detect fibres
coord1, scale1 = blob.detectFibers(
    im1.astype(np.float), diameterLimit, stepSize, tCenter, thresMagnitude
)
...
# Detect fibres
coord2, scale2 = blob.detectFibers(
    im2.astype(np.float), diameterLimit, stepSize, tCenter, thresMagnitude
)
...
ax.bar(
    (h1[1][1:] + h1[1][:-1]) / 2, h1[0], width=h1[1][1] - h1[1][0], color="r", alpha=0.3
)
ax.plot((h1[1][1:] + h1[1][:-1]) / 2, h1[0], "r")
ax.bar(
    (h2[1][1:] + h2[1][:-1]) / 2, h2[0], width=h2[1][1] - h2[1][0], color="b", alpha=0.3
)
